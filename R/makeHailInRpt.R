...
makeHailInRpt <- function(thePath = file.path("C:","DFO-MPO","PORTSAMPLING"),
  ...
  MARFISSCI.VESSELS.VESSEL_NAME,
    ...
    FROM MARFISSCI.PFISP_HAIL_IN_LANDINGS,
    ...
    (SELECT HAIL_IN_LANDING_ID, LISTAGG(DESC_ENG,', ') WITHIN GROUP (ORDER BY  RANCOR) SPECIES, LISTAGG(AREA,', ') WITHIN GROUP (ORDER BY  RANCOR) NAFO_ZONES
    FROM
    (
    SELECT
    HAIL_IN_LANDING_ID,
    ...
    WHERE
    ANAL.SSF_SPECIES_CODE =  SP.SPECIES_CODE(+) AND
    ANAL.NAFO_UNIT_AREA_ID = NAFO.AREA_ID(+)
    ) GROUP BY HAIL_IN_LANDING_ID) ALLSPEC
    ...
    AND MARFISSCI.PFISP_HAIL_IN_LANDINGS.EST_LANDING_DATE_TIME BETWEEN TRUNC(SysDate - 100) AND TRUNC(SysDate + 2)
    AND MARFISSCI.SPECIES.SPECIES_CATEGORY_ID = 1
    GROUP BY MARFISSCI.PFISP_HAIL_IN_LANDINGS.EST_LANDING_DATE_TIME,
    MARFISSCI.VESSELS.VESSEL_NAME,
    MARFISSCI.PFISP_HAIL_IN_CALLS.VR_NUMBER,
    MARFISSCI.PFISP_HAIL_OUT_LIC_DOCS.LICENCE_ID,
    MARFISSCI.COMMUNITIES.COMMUNITY_NAME
